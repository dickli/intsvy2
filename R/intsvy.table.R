  ...
  table.input <- function(variable, data, config) {
    ...
    if (config$parameters$weights == "JK") {
      ...
      tabrp <- as.matrix(sapply(1:config$parameters$BRRreps, function(i) 
        percent(as.factor(data[[variable]]), total=FALSE, 
                weights=  data[[paste(config$variables$weightBRR, i , sep="")]], na.rm=TRUE)))     
      
      # Total weighted %                                                                      
      tabtot <- percent(as.factor(data[[variable]]), weights= data[[config$variables$weightFinal]], na.rm = TRUE, total=FALSE)
      # Standard error
      if (length(tabtot)!=1) {
    ...
    result <- data.frame(table(data[[variable]]), 
    ...
    result[,"Std.err."] <- ifelse(result[,"Std.err."] == 
                                     0, NA, result[,"Std.err."])
    return(result)
  }
